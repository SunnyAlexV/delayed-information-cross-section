...
def main(folder=None):
    ...
    vs_full = np.array([F.r2(yb, curve_f[g], BENCH) for g in FINE])
    ...
    for g in GAPS:
        print(f"  g = {g:>2}:  R-squared {F.r2(yb, comb_f[g], BENCH):.4f}   "
              f"effective age {age_of(F.r2(yb, comb_f[g], BENCH), xs, vs_full):.1f} days")
    ...
    for _ in range(N_BOOT):
        ...
        vs = np.array([r2_on(yb, curve_f[k], rows, bench=BENCH) for k in FINE])
        ...
        for g in GAPS:
            a = age_of(r2_on(yb, comb_f[g], rows, bench=BENCH), xs, vs)
            draws[g].append(a)
            if np.isnan(a):
                noff[g] += 1
            elif a == 0.0:
    ...
    for g in GAPS:
        ...
        out[g] = (age_of(F.r2(yb, comb_f[g], BENCH), xs, vs_full), lo, hi,
                  nzero[g], noff[g])
