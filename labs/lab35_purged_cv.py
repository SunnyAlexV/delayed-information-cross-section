...
def main(folder=None):
    ...
    for d in DELAYS:
        ...
        if lo > 0:
            leaks.append(d)
        print(f"{d:>6}{F.r2(yb, fa, BENCH) - F.r2(yb, fb, BENCH):>+21.4f}"
    ...
    for d in DELAYS:
        ...
        for nm, _ in GAPS[2:]:
            ...
            if hi < 0:
                resid.append((d, nm))
            row += f"{F.r2(yb, fa, BENCH) - F.r2(yb, fb, BENCH):>+11.4f}" + \
    ...
    mono = []
    for d in DELAYS:
        vals = [F.r2(yb, R[(d, nm)], BENCH) for nm, _ in GAPS[1:]]
