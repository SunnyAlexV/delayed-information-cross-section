...
def main(folder=None):
    ...
    print(f"{'delta':>6}{'NOISE':>11}{'VRPus':>11}{'ORACLE':>11}{'VRPde':>11}"
          f"{'  AR(1) phi':>12}")
    for d in DELAYS:
        print(f"{d:>6}{F.r2(yb, noi[d], BENCH):>11.4f}{r2[(d, '+IV+VRPus')]:>11.4f}"
              f"{F.r2(yb, ora[d], BENCH):>11.4f}{r2[(d, '+IV+VRPde')]:>11.4f}"
              f"{phis[d]:>12.3f}")

    chk = abs(F.r2(yb, ora[0], BENCH) - r2[(0, "+IV+VRPus")])
    ...
    for d in DELAYS:
        ...
        print(f"{d:>6}{F.r2(yb, fc[(d, '+IV+VRPus')], BENCH) - F.r2(yb, noi[d], BENCH):>+16.4f}"
              + f"[{l1:>+10.5f},{h1:>+10.5f}]".rjust(26)
              + f"{r2[(d, '+IV+VRPde')] - F.r2(yb, noi[d], BENCH):>+16.4f}"
    ...
    for d in DELAYS:
        ...
        print(f"{d:>6}{F.r2(yb, f_wu, BENCH) - F.r2(yb, noi[d], BENCH):>+16.4f}"
              + f"[{l1:>+10.5f},{h1:>+10.5f}]".rjust(26)
              + f"{F.r2(yb, f_wd, BENCH) - F.r2(yb, noi[d], BENCH):>+16.4f}"
              + f"[{l2:>+10.5f},{h2:>+10.5f}]".rjust(26))
    ...
    for d in DELAYS:
        ...
        g1 = F.r2(yb, ora[d], BENCH) - r2[(d, "+IV+VRPus")]
    ...
    if dmg_sig and de_beats:
        share = [(r2[(d, "+IV+VRPde")] - r2[(d, "+IV+VRPus")]) /
                 (F.r2(yb, ora[d], BENCH) - r2[(d, "+IV+VRPus")])
                 for d in dmg_sig if F.r2(yb, ora[d], BENCH) - r2[(d, "+IV+VRPus")] > 1e-9]
        if share:
    ...
    for d in DELAYS:
        ...
        if hi < 0:
            bound.append(d)
        print(f"{d:>6}{F.r2(yb, f_lg, BENCH) - F.r2(yb, f_vn, BENCH):>+23.4f}"
    ...
    flips, material = 0, 0
    for d in DELAYS:
        g22 = F.r2(yb, fc[(d, "+IV+VRPus")], BENCH) - F.r2(yb, noi[d], BENCH)
        ...
        g05 = F.r2(yb2, f_us, BENCH) - F.r2(yb2, f_n2, BENCH)
