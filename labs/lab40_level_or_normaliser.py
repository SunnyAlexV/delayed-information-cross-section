...
def main(folder=None):
    ...
    # one benchmark per target kind: each kind defines its own label series, so
    # the trailing mean has to be taken on that series.  It does not vary with
    # delay, which is what keeps R(delta)'s denominator interpretable.
    _bench_cache = {}
    def _bench(kind):
        if kind not in _bench_cache:
            _yall = np.array([target(kind, lv, lm, t, 0) for t in range(len(lv) - H)])
            _yfull = np.full(len(lv), np.nan)
            _yfull[:len(_yall)] = _yall
            _bench_cache[kind] = L.bench_mean(_yfull, idx)
        return _bench_cache[kind]
    ...
    print("=" * 96)
    print("Section 9.1 lowers R(55) by a few points by measuring it against a control")
    print("that includes the level. If the level is arithmetic, that correction was")
    print("arithmetic too. Here the rate is recomputed inside each target, and the")
    print("size of the correction is read off this table rather than quoted.\n")
    ...
    if hi < 0.25:
        ...
        print("  volatility, and Section 9.1's rate correction goes with it.")
    elif lo > 0.75:
