...
def main(folder=None):
    ...
    own_r2 = {}
    for d in DELAYS:
        own_r2[d] = F.r2(yb, walk_own(own, y, idx, d), BENCH)
    R = {}
    ...
    curve = {g: F.r2(yb, walk_own(own, y, idx, g), BENCH) for g in fine}
    ...
    for d in DELAYS:
        ...
        for g in FACT_LAGS[1:]:
            ...
            if hi < 0:
                fhurt.append((d, g))
            row += f"{F.r2(yb, Fc[(d, g)], BENCH) - F.r2(yb, Fc[(d, 0)], BENCH):>+13.4f}" + \
                   f"{'*' if hi < 0 else ' ':>2}{'':>7}"
        print(row)
    ...
    for d in DELAYS:
        ...
        if lo > 0:
            change.append(d)
        print(f"{d:>6}{F.r2(yb, f_lag, BENCH) - F.r2(yb, f_noi, BENCH):>+15.4f}"
