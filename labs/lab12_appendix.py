...
def main(folder=None):
    ...
    print(f"  HAC bandwidth                 {bw} lags, measured (lab58 part 6); the"
          f" automatic rule would give {int(np.floor(4 * (n / 100) ** (2 / 9)))}")
    print(f"  bootstrap replicates          {N_BOOT}")
    print(f"  bootstrap block length        {L.BLOCK} days "
          f"({L.BLOCK // L.HORIZON}h), varied in part C")
    print(f"  seed                          {L.SEED}")
