...
H = L2.HORIZON
SEED = 20260913
NSEED = 20       # bootstrap seeds per crossing cell in part C2; a crossing that
                 # depends on which one is drawn is reported as knife-edge
...
def main(path=None):
  ...
  hindsight figure in prose while tabulating the expanding-window one.""")
    ...
    # ---------------- C2 -----------------------------------------------
    # Part C establishes that the constant benchmark is TWO numbers, 6.3 points
    # apart.  That matters only if the gap changes an answer, so this part asks
    # whether it does: for each forecaster, the largest delay whose 95% accuracy
    # interval lies entirely above each hurdle.
    #
    # A single bootstrap seed cannot answer that question honestly.  The first
    # version of this part used one, and reported that the threshold rule stays
    # clearly above the higher hurdle through delta = 5.  It does not: at that
    # delay the interval's lower endpoint sits within a quarter of a point of
    # the hurdle, and whether it clears depends on the seed.  Reporting the
    # winner of one draw as a crossing delay is precisely the error a bootstrap
    # exists to prevent, so the crossing is determined over SEEDS here, and a
    # delay that clears in some draws and not others is named as knife-edge
    # rather than assigned to one side.
    print("\n" + "=" * 84)
    print("C2.  DOES THE CHOICE OF CONSTANT CHANGE AN ANSWER?")
    print("=" * 84)
    print(f"For each forecaster, the largest delay whose 95% accuracy interval lies")
    print(f"entirely ABOVE each hurdle in ALL {NSEED} bootstrap seeds, and the largest")
    print("that clears it in at least one.  Where those differ the crossing is not")
    print("resolved by this sample and both are printed.  Same walk-forward, same")
    print("days, same block as the note's Table 1; only the hurdle and seed change.\n")
    roll = (rec == yb).mean() * 100.0
    expw = (np.full(len(idx), exp_maj) == yb).mean() * 100.0
    # lab02's evaluate() needs the proxy, median and feature dicts; they are
    # rebuilt here from panel()'s own arrays, so this part scores exactly the
    # series parts A to C score.
    prox20 = {"CC": vcc, "YZ": vyz}
    Ms20 = {"CC": Mcc, "YZ": Myz}
    feats20 = {t: L2.har_features(np.log(np.maximum(v, 1e-14)))
               for t, v in prox20.items()}
    ref, yb2, idx2 = L2.evaluate(y, feats20, Ms20, prox20)
    MODS = ("persist-CC", "persist-YZ", "har-CC", "har-YZ")
    clears = {}
    for mod in MODS:
        for d in L2.DELAYS:
            corr = (ref[(mod, d)] == yb2).astype(float)
            for hh in (roll, expw):
                k = 0
                for sd in range(NSEED):
                    draws = L2.block_boot(
                        corr, np.random.default_rng(SEED + 313 + 97 * sd + d))
                    k += (np.percentile(draws, 2.5) > hh / 100.0)
                clears[(mod, d, round(hh, 2))] = k

    def _span(mod, hh):
        """(all-seed crossing, any-seed crossing) as delays, None where never."""
        a = [d for d in L2.DELAYS if clears[(mod, d, round(hh, 2))] == NSEED]
        b = [d for d in L2.DELAYS if clears[(mod, d, round(hh, 2))] > 0]
        return (max(a) if a else None), (max(b) if b else None)

    print(f"{'forecaster':>13}" + "".join(
        f"{'above ' + f'{hh:.2f}%':>26}" for hh in (roll, expw)))
    knife = []
    for mod in MODS:
        cells = []
        for hh in (roll, expw):
            a, b = _span(mod, hh)
            if a == b:
                cells.append(f"through delta = {a}" if a is not None else "never")
            else:
                knife.append((mod, round(hh, 2), a, b))
                cells.append(f"{a if a is not None else 'never'}"
                             f" firm, {b} knife-edge")
        print(f"{mod:>13}" + "".join(f"{c:>26}" for c in cells))
    print(f"\n  bootstrap seeds per cell: {NSEED};  knife-edge cells "
          f"(clear in some seeds, not all): {len(knife)}")
    for mod, hh, a, b in knife:
        k = clears[(mod, b, hh)]
        print(f"    {mod} at {hh:.2f}%: delta = {b} clears in {k} of {NSEED} seeds, "
              f"so the firm crossing is {a if a is not None else 'never'}")
    diff = [m for m in MODS if _span(m, roll)[0] != _span(m, expw)[0]]
    print(f"\n  forecasters whose FIRM answer depends on which constant is used: "
          f"{len(diff)} of {len(MODS)}" + (f"  {diff}" if diff else ""))
    if diff:
        m = "persist-YZ" if "persist-YZ" in diff else diff[0]
        ar, ae = _span(m, roll)[0], _span(m, expw)[0]
        print(f"  For {m}, the note's threshold rule, the last delay firmly above")
        print(f"  the rolling-window hurdle is {ar} and above the expanding-window one")
        print(f"  is {ae}.  The higher hurdle is the harder test of the note's POSITIVE")
        print("  claim that the rule beats a constant at short delays, and the easier")
        print("  test of its NEGATIVE claim that it stops doing so.  The note must")
        print("  report both and say which claim each serves, rather than describing")
        print("  one of them as the constant-forecast level.")
    else:
        print("  The two hurdles give the same answer for every forecaster, so the")
        print("  note may quote either provided it names which one it means.")
