...
IV_EXCLUDE = {
    # The source is Cboe's own VIX_History.csv.  The two retail exports stay in
    # data/ so lab43 can keep auditing one against the other, and the nine-day
    # index lives there too - all three match the plain "vix" keyword, so all
    # three are named here.  Merging a nine-day index into a thirty-day one is
    # the VSTOXX hazard above with a different pair of files.
    "VIX":    ["9d", "vxst", "cboe_volatility_index_historical",
               "cboe volatility index historical"],
    "VIX9D":  [],
    "VDAX":   [],
...
def race(D, iv, peers, title, tags):
    ...
    for d in DELAYS:
        ...
        inc = r2(yb, f_both, BENCH) - r2(yb, f_iv, BENCH)          # foreign, given implied vol
        dl = (yb - f_iv) ** 2 - (yb - f_both) ** 2   # nested: iv inside both
        z = dl.mean() / hac_se(dl)
        rows[d] = (r2(yb, f_own, BENCH), r2(yb, f_for, BENCH), r2(yb, f_iv, BENCH), r2(yb, f_both, BENCH), inc, z)
...
def main(folder=None):
    ...
    # Same yardstick as the point estimate beside it; see lab07.
    ...
    for d in [0, 5, 21, 55]:
        ...
        inc = (e_iv - e_both).sum() / ((yb - BENCH) ** 2).sum()
        cost = (e_iv - e_sur).sum() / ((yb - BENCH) ** 2).sum()
