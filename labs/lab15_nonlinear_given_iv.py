...
def main(folder=None):
    ...
    with penalty(GRIDS["PAPER"]):
        for d in DELAYS:
            a1 = r2(yb, F[("PAPER", "IV", d)])
            a2 = r2(yb, F[("PAPER", "IV+F", d)])
            b1 = r2(yb, IV.walk(own, [iv], y, idx, d))
            b2 = r2(yb, IV.walk(own, [P, iv], y, idx, d))
    ...
    for g in GRIDS:
        ...
        for d in DELAYS:
            row = f"{d:>6}{r2(yb, F[(g, 'IV', d)]):>9.4f}"
            for a in ARMS[1:]:
                dr = r2(yb, F[(g, a, d)]) - r2(yb, F[(g, "IV", d)])
    ...
    with penalty(GRIDS["WIDE"]):
        for d in DELAYS:
            row = f"{d:>6}"
            base = [r2(yb, walk(own, S, iv, y, idx, d, "IV+F", rff)) for S in draws]
            for a in ARMS[2:]:
                c = float(np.mean([r2(yb, walk(own, S, iv, y, idx, d, a, rff)) - b0
                                   for S, b0 in zip(draws, base)]))
                net = (r2(yb, F[("WIDE", a, d)])
                       - r2(yb, F[("WIDE", "IV+F", d)])) - c
    ...
    for d in DELAYS:
        r20 = r2(yb, F[("WIDE", "IV+F+RFF20", d)])
        r60 = r2(yb, F[("WIDE", "IV+F+RFF60", d)])
