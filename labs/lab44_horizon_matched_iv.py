...
def run(D, peers, col, names, delays, label):
    ...
    return out, yb, len(idx), D.index[idx[0]], D.index[idx[-1]], BENCH
...
def main(folder=None):
    ...
    wide, ybw, nw, aw, bw, _bench_ybw = run(D.loc[full], peers, col.loc[full],
                                {"IV30": ["VIX", "VDAX"]}, DELAYS, "full window")
    narrow, ybn, nn, an, bn, _bench_ybn = run(D.loc[short], peers, col.loc[short],
                                  ARMS, DELAYS, "VIX9D window")
    print(f"\n{'delta':>6}{'+IV30 full':>13}{'+IV30 short':>14}{'difference':>13}")
    for d in DELAYS:
        a = IV.r2(ybw, wide[d]["IV30"], _bench_ybw)
        b = IV.r2(ybn, narrow[d]["IV30"], _bench_ybn)
    ...
    better = 0
    for d in DELAYS:
        r_own = IV.r2(ybn, narrow[d]["own"], _bench_ybn)
        r30 = IV.r2(ybn, narrow[d]["IV30"], _bench_ybn)
        r9 = IV.r2(ybn, narrow[d]["IV9"], _bench_ybn)
        rt = IV.r2(ybn, narrow[d]["TERM"], _bench_ybn)
    ...
    for d in DELAYS:
        ...
        for k, w in (("IV30", 12), ("IV9", 12), ("TERM", 13)):
            g = IV.r2(ybn, narrow[d]["F+" + k], _bench_ybn) - IV.r2(ybn, narrow[d][k], _bench_ybn)
    ...
    for d in DELAYS:
        yk = ybn[keep2]
        _bk = _bench_ybn[keep2]
        r30 = IV.r2(yk, narrow[d]["IV30"][keep2], _bk)
        r9 = IV.r2(yk, narrow[d]["IV9"][keep2], _bk)
        z9 = z_of(yk, narrow[d]["IV30"][keep2], narrow[d]["IV9"][keep2])
        g9 = IV.r2(yk, narrow[d]["F+IV9"][keep2], _bk) - r9
    ...
    for d in DELAYS:
        rs = IV.r2(ybn, narrow[d]["SLOPE"], _bench_ybn)
        r9 = IV.r2(ybn, narrow[d]["IV9"], _bench_ybn)
    ...
    print(f"{'delta':>6}{'R(d) foreign':>14}{'R(d) IV30':>12}{'R(d) IV9':>11}"
          f"{'R(d) TERM':>12}")
    base = IV.r2(ybn, narrow[0]["own"], _bench_ybn)
    for d in DELAYS[1:]:
        o = IV.r2(ybn, narrow[d]["own"], _bench_ybn)
        den = base - o
        f = lambda key: (f"{(IV.r2(ybn, narrow[d][key], _bench_ybn) - o) / den:.0%}"
