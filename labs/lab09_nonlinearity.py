...
def main(folder=None):
    ...
    for d in DELAYS:
        ...
        z = dl.mean() / hac_se(dl)
        cost = float(np.mean([r2(yb, walk(own, S, y, idx, d, "SQ"), BENCH)
                              - r2(yb, walk(own, S, y, idx, d, "LIN"), BENCH) for S in draws]))
        dr = r2(yb, f_sq, BENCH) - r2(yb, f_lin, BENCH)
        sq_rows[d] = (r2(yb, f_lin, BENCH), dr, z, cost)
        print(f"{d:>6}{r2(yb,f_lin, BENCH):>9.4f}{r2(yb,f_sq, BENCH):>9.4f}{dr:>+9.4f}"
    ...
    for d in DELAYS:
        ...
        z = dl.mean() / hac_se(dl)
        cost = float(np.mean([r2(yb, walk(own, S, y, idx, d, "FACTOR"), BENCH)
                              - r2(yb, walk(own, S, y, idx, d, "LIN"), BENCH) for S in draws]))
        dr = r2(yb, f_fa, BENCH) - r2(yb, f_lin, BENCH)
        fa_rows[d] = (r2(yb, f_lin, BENCH), dr, z, cost)
        print(f"{d:>6}{r2(yb,f_lin, BENCH):>9.4f}{r2(yb,f_fa, BENCH):>9.4f}{dr:>+9.4f}"
              f"{z:>8.2f}{cost:>+9.4f}{dr-cost:>+13.4f}")
