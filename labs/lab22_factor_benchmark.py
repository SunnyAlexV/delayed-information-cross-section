...
def main(folder=None):
    ...
    for d in DELAYS:
        ...
        for a in ARMS:
            ...
            if a == "PC1":
                shares[d] = sh
            row += f"{r2(yb, F[(a, d)], BENCH):>10.4f}"
        best = max(ARMS, key=lambda a: r2(yb, F[(a, d)], BENCH))
        dl = (yb - F[("FULL", d)]) ** 2 - (yb - F[("PC1", d)]) ** 2
        z = dl.mean() / hac_se(dl)
        row += f"{best:>8}{r2(yb,F[('PC1',d)], BENCH)-r2(yb,F[('FULL',d)], BENCH):>+13.4f}{z:>8.2f}{norm_p(z):>8.3f}"
        print(row)

    print(f"\n  mean variance share of the real-time first component, by delay:")
    print("   " + "  ".join(f"d={d}: {shares[d]:.1%}" for d in DELAYS))

    # ---- the two full-sample descriptives Section 5.3 quotes ----------------
    # These had been computed once by hand and typed into the paper, which is the
    # one thing this repository claims never happens.  They belong here rather
    # than in the seam-audit lab because they must sit on the SAME panel the
    # forecasting arms above use: lab03 builds its own panel with a different
    # forward-fill rule, and computing them there would answer a slightly
    # different question and disagree in the third decimal for a reason no
    # reader could see.
    # Section 5.3's figures are about the SIX peers that close before the target
    # on the same day, not all seven: Sao Paulo closes with New York and enters
    # every model a day late, so including it would answer a different question
    # from the one the sentence asks.
    D_, peers_, _lag_ = L.build(folder, TARGET)
    live = [p for p in peers_ if L.CLOSE_UTC[p] < L.CLOSE_UTC[TARGET]]
    print(f"\n  full-sample descriptives on the {len(live)} same-day peers "
          f"{live}:")
    Pl = D_[live].values
    okc = np.isfinite(Pl).all(axis=1) & np.isfinite(D_[TARGET].values)
    Xc, yc = Pl[okc], D_[TARGET].values[okc]
    A = np.column_stack([np.ones(len(Xc)), Xc])
    beta, *_ = np.linalg.lstsq(A, yc, rcond=None)
    resid = yc - A @ beta
    span = 1 - (resid @ resid) / ((yc - yc.mean()) @ (yc - yc.mean()))
    print(f"    contemporaneous R-squared of the target on them: "
          f"{span:.4f}  (n = {len(yc)})")
    print(f"    so {1 - span:.1%} of the target's current state is not linearly spanned")
    Z = (Xc - Xc.mean(0)) / (Xc.std(0) + 1e-12)
    sh = np.linalg.eigvalsh(np.cov(Z, rowvar=False))[::-1]
    sh = sh / sh.sum()
    print("    full-sample variance shares: " + "  ".join(f"{s:.1%}" for s in sh))
    print(f"    first component {sh[0]:.1%}, first two {sh[0] + sh[1]:.1%}")
    ...
    for d in DELAYS:
        c = LAB07.get(d)
        print(f"{d:>6}{r2(yb,F[('FULL',d)], BENCH):>10.4f}{r2(yb,F[('PC1',d)], BENCH):>10.4f}"
              f"{r2(yb,F[('PC1',d)], BENCH)-r2(yb,F[('FULL',d)], BENCH):>+13.4f}"
              f"{('-' if c is None else f'{c:+.4f}'):>22}")

    print("\n" + "=" * 88)
    print("VERDICT")
    print("=" * 88)
    wins = [d for d in DELAYS if r2(yb, F[("PC1", d)], BENCH) > r2(yb, F[("FULL", d)], BENCH)]
    ...
    mean_wins = [d for d in DELAYS if r2(yb, F[("MEAN", d)], BENCH) > r2(yb, F[("PC1", d)], BENCH)]
