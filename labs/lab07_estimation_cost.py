...
def r2(y, f, *, bench):
    ...
    This file computed BENCH at the top of main() and then scored through a
    helper that divided by the mean of the test period, so its own parts
    disagreed about the yardstick.  `bench` is keyword-only and mandatory so a
    missed call site raises rather than scoring against the wrong one.
    """
    return 1 - ((y - f) ** 2).sum() / ((y - bench) ** 2).sum()
...
def main(folder=None):
    ...
    for k in [1, 2, 3, 5, npeer]:
        ...
        for _ in range(reps):
            ...
            vals.append(r2(yb, walk(own, P, y, idx, 0, cols, None),
                           bench=BENCH) - r_own)
    ...
    for lam in [1.0, 10.0, 100.0, 1000.0]:
        ...
        for d in (0, 21):
            ...
            row.append(r2(yb, c, bench=BENCH) - r2(yb, o, bench=BENCH))
    ...
    for d in grid:
        ...
        for S in draws:
            ...
            e = (yb - so) ** 2 - (yb - sc) ** 2
            acc += e
            vals.append(e.sum() / ((yb - BENCH) ** 2).sum())
        dcost[d] = acc / n_draw
    ...
    # The interval must use the SAME yardstick as the point estimate beside
    # it.  This line resampled the benchmark as the mean of the resampled
    # days, while the point estimate below divides by the trailing-mean
    # benchmark, so the two were measured against different constants.
    sst_b = np.array([((yb[s] - BENCH[s]) ** 2).sum() for s in samples])
    ...
    net21 = dnet[21].sum() / ((yb - BENCH) ** 2).sum()
